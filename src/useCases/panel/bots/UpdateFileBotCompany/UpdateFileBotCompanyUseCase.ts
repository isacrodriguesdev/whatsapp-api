...
export class UpdateFileBotCompanyUseCase {
  ...
  async execute(httpRequest: HttpRequest) {
    ...
    try {
      const botConfig = await this.companyBotsRepository.getConfig(
      ...
      if (httpRequest.body.initial_message_type === "text") {
        ...
        dataFile = {
          ...dataFile,
          initial_message_file: httpRequest.file.filename,
        };
      } else if (botConfig.initial_message_file) {
        dataFile = {
          ...dataFile,
          initial_message_file: botConfig.initial_message_file,
        };
      }

      const updateResponse = await this.companyBotsRepository.updateConfig(
        httpRequest.params.id,
        {
          ...httpRequest.body,
          ...dataFile,
